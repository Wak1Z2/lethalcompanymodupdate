# 2.0.53
+ Updated code to fix issues with slight networking changes from v73.

# 2.0.52
+ Revised the rendering code for displaying reserved items on players' bodies. This may fix certain reserved items (usually modded items) from displaying their "invisible" meshes when picking up or dropping these items.<br>
Once a reserved item is dropped, layers on each renderer on these objects will revert to their original layer, instead of a hardcoded layer to prevent undesired rendering quirks.

# 2.0.51
+ Updated compatibility for LCVR version 1.4

# 2.0.50
+ Added more verbose logs to help identify some bugs, that may or may not be caused by this mod, but useful nonetheless.
